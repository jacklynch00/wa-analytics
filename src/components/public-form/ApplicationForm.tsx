...
export default function ApplicationForm({ form, hasCooldown, onSubmissionSuccess }: ApplicationFormProps) {
	...
	// Initialize multiSelectValues after form initialization
	useEffect(() => {
		const initialMultiSelectValues: Record<string, string[]> = {};
		form.questions.forEach((question) => {
			if (question.type === 'multiple_select') {
				initialMultiSelectValues[question.id] = [];
			}
		});
	...
	const renderQuestion = (question: FormQuestion) => {
		...
		// Add error handling for invalid question types
			...
			case 'text':
				return (
					<div key={question.id} className='space-y-2'>
						...
						<Input id={question.id} {...register(question.id)} placeholder={question.placeholder || 'Enter your answer...'} className={error ? 'border-red-500' : ''} />
						{error && (
			...
			case 'multiple_choice':
				return (
					<div key={question.id} className='space-y-3'>
						...
						<RadioGroup onValueChange={(value) => setValue(question.id, value)} className='space-y-2'>
				...
				<Card className='w-full max-w-2xl bg-white/70 backdrop-blur-sm border-white/60 shadow-lg'>
					<CardContent className='p-6 sm:p-8 text-center'>
						...
						<p className='text-sm sm:text-base text-gray-600 mb-4'>
							You have already submitted an application for this community recently. Please wait 5 days before submitting another application.
						</p>
						<p className='text-xs text-gray-500'>If you believe this is an error, please contact the community administrators.</p>
					</CardContent>
				</Card>
			</div>
		);
			...
			<div className='max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8'>
					...
					<CardContent>
						<form onSubmit={handleSubmit(onSubmit)} className='space-y-6'>
							...
							<div className='pt-6 border-t border-gray-200'>
								...
								<p className='text-center text-xs text-gray-500 mt-3'>By submitting this application, you agree to be contacted by the community administrators.</p>
							</div>
						</form>
					</CardContent>
				</Card>
				...
				<div className='mt-8 text-center'>
					<p className='text-sm text-gray-500'>
						Generated by{' '}
						<a href='https://usewaly.com' target='_blank' rel='noopener noreferrer' className='text-blue-600 hover:text-blue-800 underline transition-colors'>
							Waly
						</a>
					</p>
				</div>
