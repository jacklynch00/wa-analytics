...
export default function HomePage() {
  ...
  const handleFileSelect = async (file: File) => {
    ...
    try {
      ...
      sessionStorage.setItem('chatAnalysis', JSON.stringify(result.analysis, (key, value) => {
    ...
    <div className="min-h-screen bg-[var(--container-bg)]">
      {user && (
        <div className="absolute top-4 right-4 text-sm text-gray-600">
          Welcome, {user.name} | <a href="/dashboard" className="text-blue-600 hover:underline">Dashboard</a>
        </div>
      )}
      {!user && (
