...
import { Building2, Palette, AlertTriangle, Save } from 'lucide-react';
import { toast } from 'sonner';
import { AdminOnly } from '@/components/auth/RoleGuard';
import { useOrganization, useOrganizationMembers, useUpdateOrganizationName } from '@/hooks/useOrganization';
		...
		<DashboardLayout>
			<div className='space-y-8'>
				...
				<AdminOnly
					fallback={
						<Alert>
							...
							<AlertDescription>Only organization administrators can modify these settings. You are viewing in read-only mode.</AlertDescription>
						</Alert>
					}>
					<div></div>
				</AdminOnly>
						...
						<CardContent className='space-y-6'>
								...
								{isEditingName ? (
									<div className='flex items-center gap-2'>
										...
										<AdminOnly>
											...
											<Button size='sm' variant='outline' onClick={handleCancelEditName} disabled={updateOrgNameMutation.isPending}>
												Cancel
											</Button>
										</AdminOnly>
									</div>
									...
									<div className='flex items-center justify-between'>
										...
										<AdminOnly>
											<Button size='sm' variant='ghost' onClick={() => setIsEditingName(true)} className='text-gray-600 hover:text-gray-900'>
												Edit
											</Button>
										</AdminOnly>
									</div>
							...
							<div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
								...
								<div className='text-center p-4 bg-green-50 rounded-lg'>
									<div className='text-2xl font-bold text-green-600'>{members.filter((m) => m.role === 'admin').length}</div>
									<div className='text-sm text-green-700'>Administrators</div>
								</div>
								<div className='text-center p-4 bg-purple-50 rounded-lg'>
									<div className='text-2xl font-bold text-purple-600'>{new Date(organization.createdAt).toLocaleDateString()}</div>
				...
				<Card>
					...
					<CardContent className='space-y-6'>
						<div className='text-sm text-gray-600'>Customization features are coming soon! You&apos;ll be able to:</div>
				...
				<AdminOnly>
					<Card>
						...
						<CardContent className='space-y-6'>
							<Alert>
								<AlertTriangle className='w-4 h-4' />
								<AlertDescription>These settings affect your entire organization. Changes should be made carefully.</AlertDescription>
							</Alert>
							...
							<div className='space-y-4'>
								...
								<div className='space-y-2'>
									...
									<div className='text-sm text-gray-600 mb-2'>Export all organization data for backup or migration purposes.</div>
