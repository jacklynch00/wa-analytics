...
import FormTemplateSelector from '@/components/form-builder/FormTemplateSelector';
...
function FormBuilderPageContent() {
	...
	const handleTemplateSelected = (template: {
		...
		// Update form with template data, but keep the current community ID and generate a new slug
		const newForm = {
			...form,
			title: template.form.title,
			description: template.form.description || form.description,
			questions: template.form.questions.map((question: FormQuestion) => ({
				...question,
				id: question.id === 'email' ? 'email' : `question_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
			})),
			customSlug: '', // Clear slug so user can set a new one
		};
	...
	return (
		<div className='space-y-6'>
			...
			<div className='mb-6 flex flex-col sm:flex-row gap-3 justify-end'>
				<FormTemplateSelector communityId={communityId} onTemplateSelected={handleTemplateSelected} />
				<Button variant='outline' onClick={handlePreviewForm} className='w-full sm:w-auto'>
					<Eye className='w-4 h-4 mr-2' />
					Preview Form
				</Button>
				<Button onClick={handleSaveForm} disabled={saving || !form.customSlug} className='w-full sm:w-auto'>
					...
					<Card className='bg-white/70 backdrop-blur-sm border-white/60 shadow-lg'>
						<CardHeader>
							<div className='flex justify-between items-center'>
								...
								<div className='text-sm text-gray-500'>{form.questions.length}/12 questions</div>
							</div>
						</CardHeader>
						<CardContent>
							<QuestionBuilder questions={form.questions} onUpdate={updateQuestions} />
						</CardContent>
					</Card>
				</div>
			...
			{/* Preview Modal */}
			{form && <FormPreviewModal isOpen={showPreview} onClose={() => setShowPreview(false)} form={form} />}
		</div>
	);
}
