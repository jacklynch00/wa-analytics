  ...
  try {
    ...
    if (existingAnalyses >= 3) {
      return NextResponse.json(
        { error: 'Upload limit reached. You can only have 3 analyses per account.' },
        { status: 403 }
      );
    }
    ...
    try {
      ...
      fileUrl = await uploadFile(fileBuffer, fileKey, 'text/plain');
    } catch (error) {
    ...
    const chatAnalysis = await prisma.chatAnalysis.create({
      data: {
        ...
        dateRange: JSON.parse(JSON.stringify(dateRange, (key, value) => {
          if (key === 'start' || key === 'end') {
            return value instanceof Date ? value.toISOString() : value;
          }
          return value;
        })),
        analysisData: JSON.parse(JSON.stringify(analysisData, (key, value) => {
